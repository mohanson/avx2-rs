...
pub unsafe fn u64_mul_auto_avx2_unsafe(a: &[u64], b: &[u64], c: &mut [u64]) {
    for ((a, b), c) in a.iter().zip(b).zip(c) {
        *c = *a * *b;
    }
}

pub fn u64_mul_auto_avx2(a: &[u64], b: &[u64], c: &mut [u64]) {
    unsafe {
        u64_mul_auto_avx2_unsafe(a, b, c);
    }
}

#[target_feature(enable = "avx2")]
pub unsafe fn u64_mul_hand_avx2_unsafe(a: &[u64], b: &[u64], c: &mut [u64]) {
    for i in 0..a.len() / 4 {
...
pub fn u64_mul_hand_avx2(a: &[u64], b: &[u64], c: &mut [u64]) {
    unsafe {
        u64_mul_hand_avx2_unsafe(a, b, c);
    }
}
